...
def getReward(flag, apf, qBefore, q, qNext):  # 计算reward函数
    ...
    # 通信质量奖励项
    comm_reward = 0
    if apf.current_bs.los_status:
        # 视距通畅奖励
        comm_reward += 2.0
        # 信道容量奖励（归一化到0~1）
        capacity_norm = apf.current_bs.capacity / 100  # 假设最大容量100 Gbps
        comm_reward += np.clip(capacity_norm, 0, 1)
    else:
        # 非视距惩罚
        comm_reward -= 5.0

    # 切换基站惩罚
    if qBefore is not None and qBefore[0] is not None:
        prev_bs = apf.last_bs
        if prev_bs.bs_id != apf.current_bs.bs_id:
            comm_reward -= 3.0

    # 综合奖励（示例权重）
    total_reward = 0.6 * comm_reward + 0.4 * path_reward
    # if qNext[2]<=0:
    #     reward += qNext[2]*0.5
...
def transformAction(actionBefore, actionBound, actionDim):
    ...
    for i in range(actionDim):
        ...
        actionAfter.append(
            (action_i + 1) / 2 * (actionBound[1] - actionBound[0]) + actionBound[0]
        )
    return actionAfter
