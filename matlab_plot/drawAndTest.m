...
bs_pos_list = csvread("../data_csv/bs_pos_list.csv");
bs_path = csvread("../data_csv/bs_path.csv");
%% 绘图
figure(1)
...
[n, ~]=size(bs_pos_list);
for i = 1:n   %绘制基站
    drawSphere(bs_pos_list(i,:), [0.01]*n);
end
...
%         cur_bs_pos = bs_pos_list(cur_bs_id+1,:);
        plot3([pathMatrix(i,1),bs_pos_list(cur_bs_id+1,1)],[pathMatrix(i,2),bs_pos_list(cur_bs_id+1,2);],...
        [pathMatrix(i,3),bs_pos_list(cur_bs_id+1,3)],'LineWidth',1,"Color",'b');
    end
