...
from static_obstacles import Obstacle  # 几个静态障碍物环境坐标
from base_stations import BaseStation  # 基站坐标
...
class APF:
    def __init__(self):
        ...
        for i in range(self.numberOfSphere):
            self.obstacle_positions.append(self.obstacle[i, :])
        for i in range(self.numberOfCylinder):
            self.obstacle_positions.append(
                np.hstack((self.cylinder[i, :], self.cylinderH[i] / 2))
            )
        ...
        # ------------------基站------------------#
        # [2, 7, 0],[2, -3, 1],[4, 4, 0],[4, -1, 2],[5, -4, 0],[6, 5, 2],[7, 7, 0],
        self.bs_path = np.array([-1], dtype=int)  # 记录基站路径
        self.last_bs = None  # 上一个基站
        self.current_bs = None  # 当前基站
        self.bs_list = [
            BaseStation(0, [2, 0, 0]),
            BaseStation(1, [4, 0, 0]),
            BaseStation(2, [4, 1, 0]),
            ...
            BaseStation(6, [5, 6, 0]),
            BaseStation(7, [5, 8, 0]),
            BaseStation(8, [6, 10, 0]),
        ]
        self.bs_num = len(self.bs_list)  # 基站数量
        # uav_pos = np.array([2, 2, 0])  # 无人机初始位置
        self.update_communication_states(self.x0)  # 更新基站状态
        # ChannelState(bs_id=2, position=array([4., 4., 0.], dtype=float32), distance=2.8284271247461903, rx_power=-111.07147333676, snr=-27.24147674796899, capacity=0.0027203034565301334, los_status=True)

    def update_communication_states(self, uav_pos: np.ndarray):
        self.last_bs = self.current_bs  # 保存上一个基站
        """更新所有基站的信道状态"""
        self.channel_states = [
            bs.get_channel_state(uav_pos, self.obstacle_positions)
            for bs in self.bs_list
        ]
        # 选择最佳基站（例如选择SNR最高的）
        self.current_bs = max(self.channel_states, key=lambda x: x.snr)
    ...
    def calculateDynamicState(self, q):
        ...
        current_bs_state = self.bs_list[self.current_bs.bs_id].get_channel_state(
            q, self.obstacle_positions
        )
        # 为每个障碍物观测添加通信指标
        comm_metrics = np.array(
            [
                current_bs_state.rx_power,
                current_bs_state.snr,
                current_bs_state.capacity,
                1 if current_bs_state.los_status else 0,
            ]
        )
        sAll = self.qgoal - q
        for i in range(self.numberOfSphere):
            s1 = self.obstacle[i, :] - q
            dic["sphere"].append(np.hstack((s1, sAll, comm_metrics)))
        for i in range(self.numberOfCylinder):
            s1 = np.hstack((self.cylinder[i, :], q[2])) - q
            dic["cylinder"].append(np.hstack((s1, sAll, comm_metrics)))
        for i in range(self.numberOfCone):
            s1 = np.hstack((self.cone[i, :], self.coneH[i] / 2)) - q
            dic["cone"].append(np.hstack((s1, sAll, comm_metrics)))
        return dic

    def inRepulsionArea(
        self, q
    ):  # 计算一个点位r0半径范围内的障碍物索引, 返回字典{'sphere':[1,2,..],'cylinder':[0,1,..]}  2021.1.6
        dic = {"sphere": [], "cylinder": [], "cone": []}
        for i in range(self.numberOfSphere):
        ...
        for i in range(self.numberOfCone):
            if (
                self.distanceCost(
                    q[0:2], np.hstack((self.cone[i, :], self.coneH[i] / 2))
                )
                < self.r0
            ):
                dic["cone"].append(i)
    ...
    def repulsion(
        self, q, eta
    ):  # 这个版本的斥力计算函数计算的是斥力因子都相同情况下的斥力 2020.12.24被替换
        ...
        for i in range(self.obstacle.shape[0]):  # 球的斥力
            ...
            if r <= self.r0:
                tempfvec = eta * (
                    1 / r - 1 / self.r0
                ) * Rq2qgoal**2 / r**2 * self.differential(
                    q, self.obstacle[i, :]
                ) + eta * (
                    1 / r - 1 / self.r0
                ) ** 2 * Rq2qgoal * self.differential(
                    q, self.qgoal
                )
                f0 = f0 + tempfvec
            else:
    ...
    def repulsionForOneObstacle(
        self, q, eta, qobs
    ):  # 这个版本的斥力计算函数计算的是一个障碍物的斥力 2020.12.24
        f0 = np.array([0, 0, 0])  # 初始化斥力的合力
        Rq2qgoal = self.distanceCost(q, self.qgoal)
        ...
        if r <= self.r0:
            tempfvec = eta * (
                1 / r - 1 / self.r0
            ) * Rq2qgoal**2 / r**2 * self.differential(q, qobs) + eta * (
                1 / r - 1 / self.r0
            ) ** 2 * Rq2qgoal * self.differential(
                q, self.qgoal
            )
    ...
    def dynamicRepulsion(self, q):  # 动态障碍物的斥力
        ...
        if r <= self.dynamicSpherer0:
            tempfvec = self.dynamicSphereEta * (
                1 / r - 1 / self.dynamicSpherer0
            ) * Rq2qgoal**2 / r**2 * self.differential(
                q, self.dynamicSphereXYZ
            ) + self.dynamicSphereEta * (
                1 / r - 1 / self.dynamicSpherer0
            ) ** 2 * Rq2qgoal * self.differential(
                q, self.qgoal
            )
    ...
    def getqNext(
        self, epsilon, eta1List, eta2List, eta3List, q, qBefore
    ):  # eta和epsilon需要外部提供，eta1List为球的斥力列表，eta2List为圆柱体的斥力表 fix:2021.2.9
        """
        当qBefore为[None, None, None]时，意味着q是航迹的起始点，下一位置不需要做运动学约束，否则进行运动学约束
        """
        qBefore = np.array(qBefore)
        if qBefore[0] is None:
            unitCompositeForce = self.getUnitCompositeForce(
                q, eta1List, eta2List, eta3List, epsilon
            )
        ...
        self.bs_path = np.vstack(
            (self.bs_path, self.current_bs.bs_id)
        )  # 记录当前基站id
        return qNext
    ...
    def getUnitCompositeForce(self, q, eta1List, eta2List, eta3List, epsilon):
        ...
        for i in range(len(eta1List)):  # 对每个球形障碍物分别计算斥力并相加
            Repulsion = Repulsion + self.repulsionForOneObstacle(
                q, eta1List[i], self.obstacle[i, :]
            )
        for i in range(len(eta2List)):
            Repulsion = Repulsion + self.repulsionForOneObstacle(
                q, eta2List[i], np.hstack((self.cylinder[i, :], q[2]))
            )
        for i in range(len(eta3List)):
            Repulsion = Repulsion + self.repulsionForOneObstacle(
                q, eta3List[i], np.hstack((self.cone[i, :], self.coneH[i] / 2))
            )
        compositeForce = Attraction + Repulsion  # 合力 = 引力 + 斥力
        # 新增基站通信引力（朝向最佳基站）
        bs_force = (self.current_bs.position - q) * self.current_bs.capacity / 100
        compositeForce = compositeForce + bs_force

        unitCompositeForce = self.getUnitVec(
            compositeForce
        )  # 力单位化，apf中力只用来指示移动方向
    ...
    def kinematicConstrant(self, q, qBefore, qNext):
        ...
        if qBefore2q[0] != 0 or qBefore2q[1] != 0:
            x1 = np.arcsin(
                np.abs(qBefore2q[1] / np.sqrt(qBefore2q[0] ** 2 + qBefore2q[1] ** 2))
            )  # 这里计算的角限定在了第一象限的角 0-pi/2
        ...
        q2qNext = qNext - q
        x2 = np.arcsin(
            np.abs(q2qNext[1] / np.sqrt(q2qNext[0] ** 2 + q2qNext[1] ** 2))
        )  # 这里同理计算第一象限的角度
        ...
        # 约束航迹角x   xres为约束后的航迹角
        deltax1x2 = self.angleVec(
            q2qNext[0:2], qBefore2q[0:2]
        )  # 利用点乘除以模长乘积求xoy平面投影的夹角
    ...
    def checkCollision(self, q):
        ...
        for i in range(self.numberOfCylinder):  # 圆柱体的检测碰撞
            if (
                0 <= q[2] <= self.cylinderH[i]
                and self.distanceCost(q[0:2], self.cylinder[i, :]) <= self.cylinderR[i]
            ):
                return np.array([0, 1, i])
        for i in range(self.numberOfCone):
            if (
                q[2] >= 0
                and self.distanceCost(q[0:2], self.cone[i, :])
                <= self.coneR[i] - q[2] * self.coneR[i] / self.coneH[i]
            ):
    ...
    def saveCSV(self):  # 保存数据方便matlab绘图(受不了matplotlib三维了)
        ...
        self.bs_pos_list = np.array([bs.position for bs in self.bs_list])
        np.savetxt("./data_csv/bs_pos_list.csv", self.bs_pos_list, delimiter=",")
        np.savetxt("./data_csv/bs_path.csv", self.bs_path, delimiter=",", fmt="%d")
    ...
    def loop(self):  # 循环仿真
        ...
        for i in range(500):
            qNext = self.getqNext(
                self.epsilon0, eta1List, eta2List, eta3List, q, qBefore
            )  # qBefore是上一个点
            qBefore = q
            ...
            if (
                self.distanceCost(qNext, self.qgoal) < self.threshold
            ):  # 当与goal之间距离小于threshold时结束仿真，并将goal的坐标放入path
