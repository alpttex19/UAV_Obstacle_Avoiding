...
from method import choose_action, checkPath, drawActionCurve, getReward
...
if __name__ == "__main__":
    ...
    for i in range(apf.numberOfSphere):
        actorDic["sphere"].append(
            torch.load(
                "TrainedModel/Actor1.%d.pkl" % i,
                map_location=device,
            )
        )
    for i in range(apf.numberOfCylinder):
        actorDic["cylinder"].append(
            torch.load("TrainedModel/Actor2.%d.pkl" % i, map_location=device)
        )
    for i in range(apf.numberOfCone):
        actorDic["cone"].append(
            torch.load("TrainedModel/Actor3.%d.pkl" % i, map_location=device)
        )
    ...
    for i in range(500):
        ...
        apf.update_communication_states(uav_pos=q)  # 更新通信状态
        state1, state2, state3 = (
            stateq["sphere"],
            stateq["cylinder"],
            stateq["cone"],
        )  # 计算相对位置作为state
        eta1List = choose_action(
            actorDic["sphere"], state1
        )  # 通过相对位置选择引力因子和斥力因子
        ...
        q = qNext
        if (
            apf.distanceCost(q, apf.qgoal) < apf.threshold
        ):  # 如果与goal距离小于阈值则退出循环并将goal位置添加到path
            apf.path = np.vstack((apf.path, apf.qgoal))
            apf.bs_path = np.vstack((apf.bs_path, apf.current_bs.bs_id))
